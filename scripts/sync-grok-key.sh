...
if [[ "$EXPORT" == true ]]; then
  ...
  printf 'export GROK_MODEL=grok-4.5\n'
fi
...
if [[ "$USER_SECRETS" == true ]]; then
  ...
  (cd "$ROOT/src/TIKR.Api" && dotnet user-secrets set "GROK_MODEL" "grok-4.5" >/dev/null)
...
if [[ "$DOCKER_ENV" == true ]]; then
  ...
  env_file_upsert "$env_file" "GROK_MODEL" "grok-4.5"
