    ...
    ILogger<SyncfusionDocumentGenerationService> logger) : IDocumentGenerationService
{
    ...
    public async Task<GeneratedDocumentResult> CreateAgentArchivePdfAsync(
        Stream content,
        string fileName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation("Creating agent archive PDF for {File}", fileName);

        // Robust copy for reuse and position safety (dual storage callers reset streams)
        await using var memStream = new MemoryStream();
        await content.CopyToAsync(memStream, cancellationToken);
        ...
        if (ext == ".pdf")
        {
            try
            {
                using var loaded = new PdfLoadedDocument(memStream);
                foreach (PdfPageBase pdfPage in loaded.Pages)
                {
                    var g = pdfPage.Graphics;
                    // Use conservative top for loaded pages (original page size preserved; stamp is small header overlay).
                    // New PdfPage instances use GetClientSize() for precision.
                    float top = 792f - PdfMargin;

                    ApplyArchiveStamp(g, top, processedDate, fileName);
                }

                SetAiArchiveMetadata(loaded, fileName, processedDate);

                using var outStream = new MemoryStream();
                loaded.Save(outStream);
                var stampedBytes = outStream.ToArray();
                var archiveName = MakeArchiveFileName(fileName);
                logger.LogInformation("Agent archive PDF (stamped original) created: {Archive}", archiveName);
                return new GeneratedDocumentResult(stampedBytes, archiveName, "application/pdf");
            }
            catch (Exception ex)
            {
                // Best effort: preserve original bytes under archive name (no data loss)
                logger.LogWarning(ex, "Failed to load/stamp original PDF for archive; returning bytes copy under archive name");
                memStream.Position = 0;
                var origBytes = memStream.ToArray();
                var archiveName = MakeArchiveFileName(fileName);
                return new GeneratedDocumentResult(origBytes, archiveName, "application/pdf");
            }
        }

        // Non-PDF (including images for agent archive): clean stamped wrapper PDF with metadata
        using var document = new PdfDocument();
        SetAiArchiveMetadata(document, fileName, processedDate);

        var page = document.Pages.Add();
        var graphics = page.Graphics;
        var size2 = page.GetClientSize();
        float y = size2.Height - PdfMargin;
        ...
        ApplyArchiveStamp(graphics, y, processedDate, fileName);
        y -= 22;
        graphics.DrawString($"Original: {fileName}", bodyFont, PdfBrushes.Black, new PointF(PdfMargin, y));
        y -= 16;
        graphics.DrawString("Clean archive copy generated by document agent. Original file stored alongside under agent-scans/.", smallFont, PdfBrushes.DarkGray, new PointF(PdfMargin, y));
        y = PdfMargin;
        graphics.DrawString("TIKR - Town Institutional Knowledge Tracker", smallFont, PdfBrushes.Gray, new PointF(PdfMargin, y));

        var wrapperName = MakeArchiveFileName(fileName);
        logger.LogInformation("Agent archive PDF created: {Archive}", wrapperName);
        return SavePdf(document, wrapperName);
    }

    private static string MakeArchiveFileName(string originalFileName)
        => Path.ChangeExtension(Path.GetFileName(originalFileName), ".ai-archive.pdf") ?? "agent-archive.pdf";

    private static void SetAiArchiveMetadata(PdfDocumentBase document, string originalFileName, DateTime processedDate)
    {
        var baseName = Path.GetFileName(originalFileName);
        document.DocumentInformation.Title = $"AI Archive - {baseName}";
        document.DocumentInformation.Subject = "AI PROCESSED - TIKR VAULT";
        document.DocumentInformation.Author = "TIKR";
        document.DocumentInformation.Producer = "TIKR Document Agent";
        document.DocumentInformation.Keywords = $"AI PROCESSED - TIKR VAULT; {processedDate:yyyy-MM-dd HH:mm:ss} UTC; {baseName}";
    }

    private static void ApplyArchiveStamp(PdfGraphics graphics, float topY, DateTime processedDate, string originalFileName)
    {
        var stampFont = new PdfStandardFont(PdfFontFamily.Helvetica, 12, PdfFontStyle.Bold);
        var detailFont = new PdfStandardFont(PdfFontFamily.Helvetica, 8, PdfFontStyle.Regular);

        graphics.DrawString("AI PROCESSED - TIKR VAULT", stampFont, PdfBrushes.Red, new PointF(PdfMargin, topY));
        graphics.DrawString($"Processed: {processedDate:yyyy-MM-dd HH:mm:ss} UTC", detailFont, PdfBrushes.Black, new PointF(PdfMargin, topY - 16));
        graphics.DrawString($"Original: {Path.GetFileName(originalFileName)}", detailFont, PdfBrushes.Black, new PointF(PdfMargin, topY - 30));
    }

    public async Task<GeneratedDocumentResult> ConvertImageToPdfAsync(
        Stream imageContent,
        string fileName,
        CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation("Converting image to PDF for {File}", fileName);

        // Robust copy for seek/reset safety and position guarantee
        await using var memStream = new MemoryStream();
        await imageContent.CopyToAsync(memStream, cancellationToken);
        memStream.Position = 0;
        ...
        if (bitmap == null)
        {
            return CreateFallbackImagePdf(fileName);
        }
        ...
        // Stamp near top (header)
        float stampTop = size.Height - PdfMargin;
        var stampFont = new PdfStandardFont(PdfFontFamily.Helvetica, 10, PdfFontStyle.Bold);
        graphics.DrawString("AI PROCESSED - TIKR VAULT", stampFont, PdfBrushes.Red, new PointF(PdfMargin, stampTop));

        // Scale image to fit below the stamp area with margins
        float headerReserve = 50f;
        float maxWidth = size.Width - (PdfMargin * 2);
        float maxHeight = size.Height - (PdfMargin * 2) - headerReserve;
        if (maxWidth <= 0) maxWidth = 400f;
        if (maxHeight <= 0) maxHeight = 500f;

        float scale = Math.Min(maxWidth / bitmap.Width, maxHeight / bitmap.Height);
        float drawWidth = bitmap.Width * scale;
        float drawHeight = bitmap.Height * scale;
        float x = PdfMargin + (maxWidth - drawWidth) / 2;
        float y = PdfMargin + (maxHeight - drawHeight) / 2;  // vertically centered in remaining space
        ...
        using var pdfImage = new PdfBitmap(pngStream);
        ...
        logger.LogInformation("Image converted to PDF: {Out}", outName);
        return SavePdf(document, outName);
    }

    private GeneratedDocumentResult CreateFallbackImagePdf(string fileName)
    {
        using var document = new PdfDocument();
        var page = document.Pages.Add();
        var graphics = page.Graphics;
        ...
        float top = size.Height - PdfMargin;
        var titleFont = new PdfStandardFont(PdfFontFamily.Helvetica, 12, PdfFontStyle.Bold);
        var bodyFont = new PdfStandardFont(PdfFontFamily.Helvetica, 10);

        graphics.DrawString("AI PROCESSED - TIKR VAULT", titleFont, PdfBrushes.Red, new PointF(PdfMargin, top));
        graphics.DrawString($"Source: {fileName}", bodyFont, PdfBrushes.Black, new PointF(PdfMargin, top - 18));
        graphics.DrawString("Image decode fallback; original stored separately.", bodyFont, PdfBrushes.DarkGray, new PointF(PdfMargin, top - 34));
