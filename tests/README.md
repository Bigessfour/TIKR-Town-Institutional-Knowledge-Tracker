...
Licensed Grok tests (`Category=GrokLicensed`) skip when `GROK_API_KEY` / `XAI_API_KEY` is unset. Default model is **`grok-4.5`** (xAI recommended chat/code model; older `grok-3` / `grok-2-latest` may return HTTP 400).
