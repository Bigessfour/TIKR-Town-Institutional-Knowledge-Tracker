...
# Prefer plain-text license file (same as USB folder mode), then env, then optional .ps1.
$licenseTxt = Join-Path $InstallRoot "syncfusion-license.txt"
...
if (-not $env:SYNCFUSION_LICENSE_KEY -and (Test-Path $localSecrets)) {
    try { . $localSecrets } catch { Write-Warning "tikr-secrets.ps1 has a syntax error — use syncfusion-license.txt instead." }
}
